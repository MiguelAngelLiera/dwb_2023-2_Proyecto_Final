...
public class CtrlCategory{
    ...
    @PostMapping
    public ResponseEntity<ApiResponse> createCategory(@valid @RequestBody Category category, BindingResult br){
        if(br.hasErrors()){
    ...
    public ResponseEntity<ApiResponse> updateCategory(@PathVariable int category_id, @valid @RequestBody Category category, BindingResult br){
        if(br.hasErrors()){
            throw new ApiException(HttpStatus.BAD_REQUEST, br.getAllErrors().get(0).getDefaultMessage());
        }
